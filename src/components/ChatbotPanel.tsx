...
export const ChatbotPanel: React.FC = () => {
  ...
  const handleSelectPersona = (persona: Persona) => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        text: `${PERSONA_LABELS[persona]} selected. You can start with one of the suggested questions below, or ask anything directly.`,
        isUser: false,
        followUps: PERSONA_SUGGESTIONS[persona].slice(0, 3),
      },
    ]);
  };
              ...
              {selectedPersona && (
                <div className="bg-white border border-gray-200 rounded-2xl p-3 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PERSONA_SUGGESTIONS[selectedPersona].map((question) => (
                      <button
                        key={question}
                ...
                <div key={idx} className={cn("flex", msg.isUser ? "justify-end" : "justify-start")}>
                  <div className={cn(
                      ...
                      <div className="mt-2 space-y-1">
                        ...
                        {msg.sourceLinks.map((source) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="block text-xs text-blue-700 hover:underline break-words"
                          >
                            {source.label}
                          </a>
                        ))}
                    ...
                    {!msg.isUser && msg.followUps && msg.followUps.length > 0 && (
                      <div className="mt-2">
                        <p className="text-[11px] uppercase tracking-wide text-gray-500 mb-1">Suggested follow-ups</p>
                        <div className="flex flex-wrap gap-1.5">
                          {msg.followUps.slice(0, 3).map((followUp) => (
                            <button
                              key={followUp}
                              onClick={() => handleSend(followUp)}
                              className="text-[11px] px-2.5 py-1 rounded-full border border-gray-200 bg-gray-50 hover:bg-gray-100 text-gray-700"
                            >
                              {followUp}
                            </button>
                          ))}
